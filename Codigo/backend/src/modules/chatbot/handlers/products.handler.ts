  ...
  async handle(context: ChatbotContext): Promise<ChatbotResponse> {
    ...
    if (topList.length === 0) {
      return {
        ...
        replyText: [
          "Entendi, você quer consultar os produtos disponíveis.",
          "No momento, não encontrei itens ativos no catálogo para te mostrar.",
          "Próximo passo: responda 'promoções' para ver ofertas ou 'vendedor' para falar com a equipe.",
        ].join("\n"),
    ...
    const header = requestedTerm
      ? `Entendi que você quer ver opções relacionadas a "${requestedTerm}". Separei estes itens para você:`
      : "Perfeito. Separei alguns produtos disponíveis para você conferir:";

    const replyText = [
      header,
      ...topList.map((product, index) => `${index + 1}) ${product.nome} - ${toCurrency(product.preco)}`),
      "",
      "Próximo passo: responda com o número do item, por exemplo 1, ou diga 'quero o primeiro'.",
    ].join("\n");
